...
const check = (p: number, q: number, len: number, direction: number, element: string): boolean => {
  let count: number = 0;
  switch (direction) {
    case 0:
      if (q + len <= GRID_SIZE) {
...
const letters:string[]= [...letters_set];
console.log(letters)
...
window.onload = () => {
  ...
  const activate = (b: HTMLButtonElement) => b.classList.add("active");
  const deactivate = (b: HTMLButtonElement) => b.classList.remove("active");
  const fix = (b: HTMLButtonElement) => b.classList.add("fix");
  const completed = (arr: [HTMLButtonElement, Point][]) => arr.forEach(e => fix(e[0]));
  const popAllElements = (arr: [HTMLButtonElement, Point][]) => arr.forEach(e => deactivate(e[0]));

  const isPartialWord = (s: string): boolean => {
    for (const w of inputArray) {
      if (w.startsWith(s)) return true;
      const rev = w.split("").reverse().join("");
      if (rev.startsWith(s)) return true;
    }
    return false;
  };
      ...
      btn.addEventListener("click", () => {
        ...
        if (curr_array.length === 1) {
          ...
          if (maxAbs !== 1 || (dI === 0 && dJ === 0)) {
            ...
            curr_array = [[btn, [i, j]]];
            stepDelta = null;
            if (isCompleteWord(head)) { completed(curr_array); head=""; curr_array=[]; stepDelta=null; }
            return;
          }
